...
При этом в случае выполнения отмены установки конкретной версии модуля
( параметр INSTALL_VERSION не равен Last), может потребоваться явно
указать версию модуля, получающуюся в БД после отмены установки, с помощью
параметра <UNINSTALL_RESULT_VERSION>:
