...
/* ivar: isDboutOutput
  Признак вывода сообщений лога через пакет dbms_output.
*/
isDboutOutput boolean := null;

/* ivar: isTableOutput
  Признак вывода сообщений лога в таблицу <lg_log>.
*/
  ...
  if destinationCode is null then
    isDboutOutput := coalesce(
      ...
      pkg_Common.isProduction() = 0
        -- сессия не запущена через dbms_job
...
begin
  ...
  if isDboutOutput is null then
    -- Устанавливаем значения вывода по умолчанию
