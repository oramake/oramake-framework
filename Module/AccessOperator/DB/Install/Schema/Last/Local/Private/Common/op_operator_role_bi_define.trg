...
-- Триггер для инициализации значений перед вставкой записи в таблицу <op_operator_role>

create or replace trigger op_operator_role_bi_define
  before insert
  on op_operator_role
  for each row
-- op_operator_role_bi_define
begin
  -- Тип действия
  if :new.action_type_code is null then
    :new.action_type_code := pkg_AccessOperator.CreateOperatorRole_ActTpCd;
  end if;  
...
end op_operator_role_bi_define;
/