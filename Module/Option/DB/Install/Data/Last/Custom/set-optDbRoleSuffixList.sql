...
-- содержащим настройки ( по одной записи для каждой БД).
--
-- Колонки курсора:
-- production_db_name         - Имя промышленной БД
-- local_role_suffix          - Суфикс ролей, выдающих права на все параметры,
--                              созданные в данной БД
--                              ( см.
...
from
  ...
  from
    table( cmn_string_table_t(
      -- Настройки в формате "<production_db_name>[:<local_role_suffix>]",
      -- ( пробелы между элементами игнорируются, если local_role_suffix
      -- не задан, то ему будет назначено значение по-умолчанию ( см. выше))
      'ProdDb: Prod'
      , 'ProdDb2'
      --
    )) t
