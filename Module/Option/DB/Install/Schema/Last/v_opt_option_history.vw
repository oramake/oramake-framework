...
comment on column v_opt_option_history.object_short_name is
  ' ороткое название объекта модул€ ( уникальное в рамках модул€), к которому относитс€ параметр ( null если не требуетс€ разделени€ параметров по объектам либо параметр относитс€ ко всему модулю)'
/
comment on column v_opt_option_history.object_type_id is
  'Id типа объекта'
/
comment on column v_opt_option_history.option_short_name is
  ' ороткое название параметра ( уникальное в рамках модул€ либо в рамках объекта модул€, если заполнено поле object_short_name)'
/
...
comment on column v_opt_option_history.old_option_short_name is
  '”старевшее поле:  ороткое название параметра в таблице opt_option'
/
comment on column v_opt_option_history.old_mask_id is
  '”старевшее поле: Id маски дл€ значени€ параметра'
/
comment on column v_opt_option_history.old_option_name_test is
  '”старевшее поле: Ќазвание тестового параметра в таблице opt_option'
/
