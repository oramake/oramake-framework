-- trigger: opt_option_new_bu_history
-- При изменении записи в <opt_option_new> добавляет историческую запись со
-- старыми данными в <opt_option_history>.
create or replace trigger opt_option_new_bu_history
  before update
  on opt_option_new
  for each row
declare
...
begin
  ...
  hs.old_option_short_name          := :old.old_option_short_name;
  hs.old_mask_id                    := :old.old_mask_id;
  hs.old_option_name_test           := :old.old_option_name_test;
