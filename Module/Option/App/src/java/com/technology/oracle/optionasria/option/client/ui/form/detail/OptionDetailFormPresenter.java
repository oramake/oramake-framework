...
public class OptionDetailFormPresenter<E extends OptionEventBus, S extends OptionServiceAsync>
  ...
  public void onSetCurrentRecord(SetCurrentRecordEvent event) {
    ...
    event.getCurrentRecord().set(CURRENT_DATA_SOURCE, storage.getItem(CURRENT_DATA_SOURCE));
  ...
  public void onDoGetRecord(DoGetRecordEvent event) {
    ...
    record.set(CURRENT_DATA_SOURCE, storage.getItem(CURRENT_DATA_SOURCE));
...
*/
		 ...
		 */
    fields.addFieldListener(VALUE_TYPE_CODE, JepEventType.FIRST_TIME_USE_EVENT, event -> service.getValueType(Storage.getSessionStorageIfSupported().getItem(CURRENT_DATA_SOURCE), new FirstTimeUseAsyncCallback<List<JepOption>>(event) {
      public void onSuccessLoad(List<JepOption> result){
  ...
  private void objectTypes(JepOption dataSource){
    ...
    if(!JepRiaUtil.isEmpty(dataSource)){
      ...
      service.getObjectType(Storage.getSessionStorageIfSupported().getItem(CURRENT_DATA_SOURCE), new JepAsyncCallback<List<JepOption>>() {
        //public void onSuccessLoad(List<JepOption> result){
  ...
  private void changeModules(JepOption dataSource){
    ...
    if(!JepRiaUtil.isEmpty(dataSource)){
      ...
      service.getModule(Storage.getSessionStorageIfSupported().getItem(CURRENT_DATA_SOURCE), new JepAsyncCallback<List<JepOption>>() {
        //public void onSuccessLoad(List<JepOption> result){
        public void onSuccess(List<JepOption> result) {
