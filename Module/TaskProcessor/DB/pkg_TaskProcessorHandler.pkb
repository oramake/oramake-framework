    ...
    cursor curActiveTask is
      ...
      where
        ...
        or a.sid is not null
          -- с начала выполнения прошло не менее 2-х секунд (для исключения
          -- ложного срабатывания при строго одновременном старте обработчиков
          -- за счет задержки обновления данных в v$session)
          and a.start_order_date <= sysdate - 2/86400
          and not exists
            (
            select
              null
