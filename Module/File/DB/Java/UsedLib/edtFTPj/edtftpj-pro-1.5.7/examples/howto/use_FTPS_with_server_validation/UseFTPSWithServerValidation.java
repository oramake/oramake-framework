...
public class UseFTPSWithServerValidation {
    ...
    public static void main(String[] args) {
        ...
        if (args.length < 4) {
            System.out
                    .println("Usage: run remote-host username password server-cert-file [hostname-checking]");
            System.out
                    .println("hostname-checking = 'on' or 'off'");
            System.exit(1);
        }
        ...
        boolean enableHostNameChecking = args.length < 5
                || args[4].toLowerCase().equals("on");
        String filename = "UseFTPSWithServerValidation.java";

        // set up logger so that we get some output
        Logger log = Logger.getLogger(UseFTPSWithServerValidation.class);
        ...
        try {
            ...
            if (!enableHostNameChecking) {
                ...
                log
                        .info("Disable host-name checking (only recommended when testing)");
